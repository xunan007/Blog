...
### HTTP1.1的合并请求、散列域名是否适用于HTTP2.0

首先，答案是“没有必要”。之所以没有必要，是因为这跟`HTTP2.0`的头部压缩有很大的关系。

首先，头部压缩技术中，客户端和服务器均会维护两份相同的静态字典和动态字典。
